...
void Set_RGB_D3_Color(color) {          // cycle between the RGB colors for D3 
...
{  
    ...
    tempSecond = 0xff;
    Set_RGB_D3_Color(0);                                        // set RGB D3 to off
    while (1)
    {
        DS3231_Read_Time();                                     
        ...
        if (check_for_button_input() == 1)                  
        {
            ...
            switch (found)
            {
                case Setup_Time_Key:                        // call the setup time key and light up the key pressed led
                    KEY_PRESSED = 1;
                    Do_Beep_Good();
                    KEY_PRESSED = 0;
                    Do_Setup_Time();
                ...
                case Setup_Alarm_Key:                       // call the setup alarm key and light up the key pressed led
                    KEY_PRESSED = 1;
                    Do_Beep_Good();
                    KEY_PRESSED = 0;
                    Do_Setup_Alarm_Time();
                ...
                case Setup_Fan_Temp_Key:                    // call the setup fan temp key and light up the key pressed led
                    KEY_PRESSED = 1;
                    Do_Beep_Good();
                    KEY_PRESSED = 0;
                    Setup_Temp_Fan();            
                ...
                case Toggle_Fan_Key:                        // call the toggle fan key and light up the key pressed led
                    KEY_PRESSED = 1;
                    Do_Beep_Good();
                    KEY_PRESSED = 0;
                    Toggle_Fan_Monitor();
                ...
                default:                                    // turn on the key pressed led
                    KEY_PRESSED = 1;
                    Do_Beep_Bad();
                    KEY_PRESSED = 0;
                break;
        ...
        {
            INT0_flag = 0;                  
            if (ALARMEN == 0) ALARMEN = 1;                  // if alarmen == 0 set alarmen to 1
            else ALARMEN = 0;
        }
        ...
        {            
            if(RTC_ALARM_NOT == 0)
            {
                ...
                {    
                    ...
                    if (volt > 2.5)                             // turn off alarm using the daylight sensor
                    {
                        DS3231_Turn_Off_Alarm();
                        alarm_mode = 0;
                        MATCHED = 0;
