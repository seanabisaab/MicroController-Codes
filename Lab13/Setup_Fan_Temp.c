...
void Setup_Temp_Fan(void)
{
    ...
    {   
         if (check_for_button_input() == 1)
        {
             ...
             {                                      // Set up the given buttons for the proper buttons for changing the temp
                Do_Beep_Good();                     // do a good beep
...
{
	if(setup_fan_set_temp <110){                // if less than 110 increment the fan temp      `   
        setup_fan_set_temp++;
    }
    else 
    {
        setup_fan_set_temp=110;                 // cap at 110
    }
...
{
	if(setup_fan_set_temp >50)                  // if greater than 50 decrement fan temp
    {
        setup_fan_set_temp--;
    }
    else                                        // set the floor to 50
    {
...
{ 
    ...
    strcpy(txt, "ECE3301L S1 Final\0");                    // Text displayed 
